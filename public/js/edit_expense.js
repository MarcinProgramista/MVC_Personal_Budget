...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (typeof window.updateBalanceUI !== "function") {
        window.updateBalanceUI = function (sum, incomesCount = 0, expensesCount = 0) {
    ...
    editButtonsExpense.forEach(button => {
        button.addEventListener("click", () => {
            ...
            if (document.getElementById("editExpenseCategory")) document.getElementById("editExpenseCategory").value = categoryId;
            if (document.getElementById("editExpensePayment")) document.getElementById("editExpensePayment").value = paymentId;
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if (li) {
            ...
            if (dateDiv) dateDiv.innerHTML = `<i class="fas fa-calendar-alt me-1"></i>${new Date(formData.date).toLocaleDateString("en-US")}`;

            const paymentDiv = li.querySelector(".text-primary");
            if (paymentDiv) paymentDiv.innerHTML = `<i class="bi bi-credit-card me-1"></i>${formData.name_payment}`;
        }

        // SUMY — używamy danych z serwera (totals)
        if (document.getElementById("sumDetailsExpense")) {
            document.getElementById("sumDetailsExpense").textContent = `${data.totals.sumAllExpenses} PLN`;
        }
        if (document.getElementById("sumALlExpensesTop")) {
            document.getElementById("sumALlExpensesTop").textContent = `${data.totals.sumAllExpenses} PLN`;
        }
    ...
    function refreshExpenseList(expenses) {
        const list = document.getElementById('expenseBalanceCategoriesList') || document.getElementById('expenseDetailsBalanceCategoriesList');
        if (!list) return;
        list.innerHTML = '';
